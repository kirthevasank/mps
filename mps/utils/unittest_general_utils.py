...
class GeneralUtilsTestCase(BaseTestClass):
  ...
  def test_generate_01_grid(self):
    """ Tests for draw gaussian samples. """
    self.report('Testing generate_01_grid.')
    grid_data = [(2, 3), (3, 4), (5, 2)]
    for grid_size, dim in grid_data:
      grid = general_utils.generate_01_grid(grid_size, dim)
      if grid.shape[0] < 10:
        self.report('Grid for (n, d) = (%d, %d): %s.'%(grid_size, dim, grid))
      assert grid.shape == (grid_size ** dim, dim)
