"""
  Unit tests for exd_utils.
  -- kandasamy@cs.cmu.edu
"""

import numpy as np
# Local imports
from exd import exd_utils
...
class EDUtilsTestCase(BaseTestClass):
  ...
  def test_latin_hc_indices(self):
    ...
    for data in self.lhs_data:
      ...
      lhs_idxs = exd_utils.latin_hc_indices(data[0], data[1])
  ...
  def test_latin_hc_sampling(self):
    ...
    for data in self.lhs_data:
      ...
      lhs_samples = exd_utils.latin_hc_sampling(data[0], data[1])
  ...
  def test_random_sampling(self):
    ...
    for data in self.lhs_data:
      self._check_sample_sizes(data, exd_utils.random_sampling_cts(data[0], data[1]))

  def test_random_sampling_kmeans(self):
    """ Tests random sampling with k-means. """
    self.report('Test random sampling with k-means.')
    for data in self.lhs_data:
      self._check_sample_sizes(data, exd_utils.random_sampling_kmeans_cts(data[0],
                                                                         data[1]))
